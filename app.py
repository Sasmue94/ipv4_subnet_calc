...
with st.form(key="calculator_form", enter_to_submit=False):
    ...
    current_prefix = st.number_input("current Prefix", min_value=0, max_value=32, value=24, step=1, key="current_prefix")
    target_prefix = st.number_input("target Prefix", min_value=0, max_value=32, value=25, step=1, key="target_prefix")
    ...
    if submit:
        ...
        ip = IPv4(ip=address, prefix=current_prefix)
        st.divider()
